...
from taskweaver.tw_logging import TelemetryLogger
...
class CodeGenerator(Role):
    ...
    def __init__(
        ...
        self.compression_template = read_yaml(self.config.compression_prompt_path)[
            "content"
        ]
        ...
        for i, example in enumerate(self.examples):
            chat_history.extend(
                self.compose_conversation(
                    example.rounds, example.plugins, add_requirements=False
                ),
            )
    ...
    def format_attachment(self, attachment: Attachment):
        if (
            attachment.type == AttachmentType.thought
            and "{ROLE_NAME}" in attachment.content
        ):
        ...
        for round_index, conversation_round in enumerate(rounds):
            for post_index, post in enumerate(conversation_round.post_list):
                ...
                is_final_post = (
                    ...
                    and post_index == len(conversation_round.post_list) - 1
                )
                if is_first_post:
                    user_message = (
                        self.conversation_head_template.format(
                            ...
                            PLUGINS=(
                                "None"
                                if len(plugins) == 0
                                else self.format_plugins(plugins)
                            ),
                            ROLE_NAME=self.role_name,
                        )
                        + "\n"
    ...
    def reply(
        ...
        **kwargs: ...,
        ...
        self.tracing.set_span_attribute(
            "enable_auto_plugin_selection", self.config.enable_auto_plugin_selection
        )
...
def format_code_feedback(post: Post) -> str:
    ...
    for attachment in post.attachment_list:
        if (
            attachment.type == AttachmentType.verification
            and attachment.content == "CORRECT"
        ):
            feedback += "## Verification\nCode verification has been passed.\n"
            verification_status = "CORRECT"
        elif (
            attachment.type == AttachmentType.verification
            and attachment.content == "NONE"
        ):
            feedback += "## Verification\nNo code verification.\n"
            verification_status = "NONE"
        elif (
            attachment.type == AttachmentType.verification
            and attachment.content == "INCORRECT"
        ):
            feedback += (
            ...
            attachment.type == AttachmentType.code_error
            and verification_status == "INCORRECT"
        ):
            feedback += f"{attachment.content}\n"
        elif (
            attachment.type == AttachmentType.execution_status
            and attachment.content == "NONE"
        ):
            feedback += "## Execution\nNo code execution.\n"
...
from src.smol_loop import SmolLoop
from src.tools import CodeAnalysisTool, CodeGenerationTool

# Load environment variables
load_dotenv()

@pytest.fixture
async def smol_system(tmp_path):
    """Create and initialize SMOL system for testing"""
    system = SmolLoop(model_dir=str(tmp_path / "models"))
...
@pytest.mark.asyncio
async def test_code_analysis(smol_system, test_code, tmp_path):
    ...
    # Run analysis
    analysis = await smol_system.analyze_codebase(tmp_path)
    
    assert "analyses" in analysis
    assert len(analysis["analyses"]) > 0
    
    first_analysis = analysis["analyses"][0]
    assert "file" in first_analysis
    assert "improvements" in first_analysis
    assert "metrics" in first_analysis
    assert "complexity" in first_analysis["metrics"]
    assert "patterns" in first_analysis["metrics"]

@pytest.mark.asyncio
async def test_code_generation(smol_system, tmp_path):
    ...
    # Run SMOL loop
    improvements = await smol_system.run(test_dir, iterations=1)
    ...
    for imp in improvements:
        ...
        assert "iteration" in imp
        assert "metrics" in imp

@pytest.mark.asyncio
async def test_code_analyzer_tool():
    """Test CodeAnalysisTool directly"""
...
def complex_function(x, y):
    ...
    try:
        # Run analysis
        result = await analyzer.run({"file_path": test_file})
...
@pytest.mark.asyncio
async def test_code_generator_tool(tmp_path):
    """Test CodeGenerationTool directly"""
    generator = CodeGenerationTool(model_dir=str(tmp_path))
    
    # Test code generation
    result = await generator.run({
        "prompt": "Create a function to calculate factorial",
        "output_file": str(tmp_path / "factorial.py")
    })
